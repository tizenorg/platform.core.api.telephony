...
int telephony_network_get_base_station_id(telephony_h handle, int *base_id)
{
	...
	tapi_h = ((telephony_data *)handle)->tapi_h;
	CHECK_INPUT_PARAMETER(tapi_h);
	CHECK_INPUT_PARAMETER(base_id);

	ret = __get_serving_network(tapi_h, &data);
	if (ret == TAPI_API_SUCCESS) {
		*base_id = data.info.cdma_info.base_station_id;
		LOGI("base_id:[%d]", *base_id);
		ret = TELEPHONY_ERROR_NONE;
	} else if (ret == TAPI_API_ACCESS_DENIED) {
		LOGE("PERMISSION_DENIED");
		ret = TELEPHONY_ERROR_PERMISSION_DENIED;
...
int telephony_network_get_base_station_latitude(telephony_h handle, int *base_lat)
{
	...
	tapi_h = ((telephony_data *)handle)->tapi_h;
	CHECK_INPUT_PARAMETER(tapi_h);
	CHECK_INPUT_PARAMETER(base_lat);

	ret = __get_serving_network(tapi_h, &data);
	if (ret == TAPI_API_SUCCESS) {
		*base_lat = data.info.cdma_info.base_station_latitude;
		LOGI("base_lat:[%d]", *base_lat);
		ret = TELEPHONY_ERROR_NONE;
	} else if (ret == TAPI_API_ACCESS_DENIED) {
		LOGE("PERMISSION_DENIED");
		ret = TELEPHONY_ERROR_PERMISSION_DENIED;
...
int telephony_network_get_base_station_longitude(telephony_h handle, int *base_long)
{
	...
	tapi_h = ((telephony_data *)handle)->tapi_h;
	CHECK_INPUT_PARAMETER(tapi_h);
	CHECK_INPUT_PARAMETER(base_long);

	ret = __get_serving_network(tapi_h, &data);
	if (ret == TAPI_API_SUCCESS) {
		*base_long = data.info.cdma_info.base_station_longitude;
		LOGI("base_long:[%d]", *base_long);
		ret = TELEPHONY_ERROR_NONE;
	} else if (ret == TAPI_API_ACCESS_DENIED) {
		LOGE("PERMISSION_DENIED");
		ret = TELEPHONY_ERROR_PERMISSION_DENIED;
