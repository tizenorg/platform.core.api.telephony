...
int main()
{
	...
	int tac;
	int nid, sid, base_id, base_lat, base_long;
	...
	ret_value = telephony_network_get_base_station_id(handle_list.handle[0], &base_id);
	if (ret_value != TELEPHONY_ERROR_NONE)
		LOGE("telephony_network_get_base_station_id() failed!!! [%d]", ret_value);
	else
		LOGI("Base station ID is [%d]", base_id);

	ret_value = telephony_network_get_base_station_latitude(handle_list.handle[0], &base_lat);
	if (ret_value != TELEPHONY_ERROR_NONE)
		LOGE("telephony_network_get_base_station_latitude() failed!!! [%d]", ret_value);
	else
		LOGI("Base station latitude is [%d]", base_lat);

	ret_value = telephony_network_get_base_station_longitude(handle_list.handle[0], &base_long);
	if (ret_value != TELEPHONY_ERROR_NONE)
		LOGE("telephony_network_get_base_station_longitude() failed!!! [%d]", ret_value);
	else
		LOGI("Base station longitude is [%d]", base_long);
